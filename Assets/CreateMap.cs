...
public class CreateMap : TimeBehaviour {
    ...
    public override IEnumerator TimeAction(Action callback = null)
    {
        ...
        int n = 0;
        StartCoroutine(TweenTransform.Tween(playerCube, mm.GetStartPosition(), Vector3.one * 0.9f, Vector3.zero, 0.2f));
        ...
        for (int x = 0; x < mm.currentSize.x; x++)
        {
            for (int y = 0; y < mm.currentSize.y; y++)
            {
                for (int z = 0; z < mm.currentSize.z; z++)
                {
                    ...
                    StartCoroutine(TweenTransform.Tween(cube.transform, pos, Vector3.one, Vector3.zero, 0.4f));
                }
