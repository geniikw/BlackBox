...
public class MapAnimation : TimeBehaviour {
    ...
    public override IEnumerator TimeAction(System.Action callback)
    {
        ...
        {
            StartCoroutine(TweenTrans.Position(listChild[n], listChildDefaultPosition[n], 0.5f));
            yield return wait;
        }
    }
    ...
    public IEnumerator Victory()
    {
        ...
        {
            StartCoroutine(TweenTrans.Position(child, child.position+Random.onUnitSphere * 10f, 3f));
    ...
    public IEnumerator Lose()
    {
        foreach (var child in listChild)
        {
            StartCoroutine(TweenTrans.Position(child,  child.transform.position +Vector3.down*10f, 3f));
            Destroy(child.gameObject, 3f);
        }
        yield return new WaitForSeconds(3f);
        listChild.Clear();
